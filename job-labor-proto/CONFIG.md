...
## ⚠️ ВАЖНО: Бэкенд и Mock Server

Домен `api.workmatch.dev` (используемый по умолчанию) **не доступен**. Это placeholder.

Для локальной разработки **необходимо** использовать встроенный Mock Server:
...
2. Настройте `.env` файл для использования локального сервера:
   ```env
